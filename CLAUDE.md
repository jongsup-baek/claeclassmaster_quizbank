...
- id: {과목}_{강의번호}_{순번}     # 예: svbasic_02_001
  ...
  code: |                           # has_code: true일 때만
    코드 내용
  has_image: false                  # 이미지 포함 여부
  image: "./images/classNN_X_Y.drawio.svg"  # has_image: true일 때만, 교재 원본 경로
  image_alt: "이미지 설명"          # has_image: true일 때만
  explanation:
    - "해설 포인트 1"
    - "해설 포인트 2"
    - "해설 포인트 3"
...
`principles/release_workflow.md` 참조. 핵심:
- **릴리즈** = lecture marp → `build_deck.py --both` → PPTX/PDF (class 레벨)
- **lab 업데이트** = GitHub class org template/학생 레포 push (build_deck.py 대상 아님)
